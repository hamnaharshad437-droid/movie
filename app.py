...
st.markdown(
    ...
    .brand-icon {
        font-size: 2rem;
        background: linear-gradient(135deg, #1677FF, #123B73);
        width: 48px;
        height: 48px;
        display: flex;
        align-items: center;
    ...
    .stButton > button {
        ...
        border: 1px solid rgba(45, 140, 255, 0.4) !important;
    ...
    .stTextInput > div > div > input:focus {
        border-color: var(--bright-blue) !important;
        box-shadow: 0 0 0 2px rgba(45, 140, 255, 0.25) !important;
    }
    ...
    .movie-card-container:hover {
        ...
        box-shadow: 0 10px 25px rgba(22, 119, 255, 0.25);
    }
    ...
    .details-overlay {
        position: absolute;
        inset: 0;
        background: linear-gradient(to top, #05070D 10%, rgba(8, 17, 31, 0.7) 60%, rgba(5, 7, 13, 0.3) 100%);
    }
...
def render_movie_grid(movies: List[Dict[str, Any]], cols_per_row: int = 4, key_prefix: str = ""):
    if not movies:
        st.markdown(
            """
            <div style="text-align: center; padding: 2.5rem 1rem; color: #A7B0C0; background: #0B1628; border-radius: 12px; border: 1px dashed #1A2B45;">
                <div style="font-size: 2.5rem; margin-bottom: 8px;">🎬</div>
                <h3 style="color: #FFFFFF; margin-bottom: 4px;">No movies found</h3>
...
def render_movie_details(movie_id: int):
    ...
    with c_post:
        st.markdown(
            f"""
            <div style="border-radius: 12px; overflow: hidden; border: 2px solid #1A2B45; box-shadow: 0 10px 30px rgba(0,0,0,0.7); margin-top: -80px; position: relative; z-index: 10;">
    ...
    with c_info:
        ...
        if tagline:
            st.markdown(f"<p style='color: #2D8CFF; font-style: italic; font-size: 1.05rem;'>\"{tagline}\"</p>", unsafe_allow_html=True)
        st.markdown(
            f"""
            <div style="display: flex; align-items: center; gap: 14px; flex-wrap: wrap; margin-bottom: 16px;">
                <span class="rating-badge" style="font-size: 0.95rem; padding: 4px 10px;">⭐ {rating:.1f} ({vote_count:,} votes)</span>
                <span style="color: #A7B0C0;">📅 {release_date}</span>
                <span style="color: #A7B0C0;">⏱️ {runtime}</span>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
with st.sidebar.expander("🔑 TMDB API Key", expanded=not is_api_configured()):
    if is_api_configured():
        st.markdown(
            """
            <div style="background: rgba(22, 119, 255, 0.15); border: 1px solid #1677FF; padding: 10px; border-radius: 8px; font-size: 0.82rem;">
        ...
        st.markdown(
            """
            <div style="background: rgba(255, 184, 0, 0.1); border: 1px solid #FFB800; padding: 10px; border-radius: 8px; font-size: 0.82rem; margin-bottom: 8px;">
...
with st.sidebar.expander("📥 Download App Files", expanded=False):
    st.markdown("<p style='font-size: 0.8rem; color: #A7B0C0;'>Download source files directly:</p>", unsafe_allow_html=True)
    try:
...
st.sidebar.markdown(
    """
    <div style="font-size: 0.75rem; color: #A7B0C0; text-align: center; margin-top: 2rem;">
...
if st.session_state.selected_movie_id is not None:
    ...
    st.markdown("<p style='color: #A7B0C0; margin-top: -10px;'>Films capturing global audience attention.</p>", unsafe_allow_html=True)
    ...
    st.markdown("# ⭐ Popular Movies")
    st.markdown("<p style='color: #A7B0C0; margin-top: -10px;'>Worldwide most-watched films right now.</p>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='color: #A7B0C0; margin-top: -10px;'>Catch the latest theatrical screenings.</p>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='color: #A7B0C0; margin-top: -10px;'>Highest rated cinema masterpieces of all time.</p>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='color: #A7B0C0; margin-top: -10px;'>Select any genre to explore curated titles.</p>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='color: #A7B0C0; margin-top: -10px;'>Search by title or storyline keywords.</p>", unsafe_allow_html=True)
    ...
    st.markdown("<p style='color: #A7B0C0; margin-top: -10px;'>Your saved collection of movies.</p>", unsafe_allow_html=True)

    wl = st.session_state.watchlist
    if not wl:
        st.markdown(
            """
            <div style="text-align: center; padding: 4rem 1.5rem; background: #0B1628; border-radius: 16px; border: 1px dashed #1A2B45; margin-top: 1.5rem;">
                <div style="font-size: 3.5rem; margin-bottom: 12px;">🍿</div>
                <h2 style="color: #FFFFFF; margin-bottom: 8px;">Your Watchlist is Empty</h2>
                <p style="color: #A7B0C0; max-width: 480px; margin: 0 auto 1.5rem auto;">
    ...
    st.markdown("<p style='color: #A7B0C0; margin-top: -10px;'>Discover movies tailored to your mood and watchlist taste.</p>", unsafe_allow_html=True)
    ...
    st.markdown(
        """
        <div style="background: #0B1628; border: 1px solid #1A2B45; border-radius: 14px; padding: 2rem; margin-top: 1rem;">
            <h2 style="color: #FFFFFF; margin-bottom: 8px;">🎬 Movie Hub</h2>
            <p style="color: #2D8CFF; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px; font-size: 0.85rem;">
                Discover • Explore • Watch
            </p>
            <p style="color: #E2E8F0; line-height: 1.7; margin-top: 1rem;">
            ...
            <hr style="border-color: #1A2B45; margin: 1.5rem 0;" />
            <h4 style="color: #FFFFFF; margin-bottom: 8px;">⚖️ Data Attribution & Disclaimer</h4>
            <p style="font-size: 0.85rem; color: #A7B0C0; line-height: 1.6;">
